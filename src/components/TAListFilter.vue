<template>
    <div class="px-2 pt-2 d-flex">
        ...
        <MonitorListFilterDropdown :filterActive="filterState.parents?.length > 0">
            <template #status>
                <Group
                    v-if="filterState.parents?.length === 1"
                    :item="parentsList.find(parent => parent.id === filterState.parents[0])"
                    :size="'sm'"
                />
                <span v-else>
                    {{ $t('Group') }}
            ...
            <template #dropdown>
                ...
                <li v-for="parent in parentsList" :key="parent.id">
                    <div class="dropdown-item" tabindex="0" @click.stop="toggleParentFilter(parent)">
                        <div class="d-flex align-items-center justify-content-between">
                            <span><Group :item="parent" :size="'sm'" /></span>
                            <span class="ps-3">
                                {{ getGroupMonitorCount(parent) }}
                                <span v-if="filterState.parents?.includes(parent.id)" class="px-1 filter-active">
                                    <font-awesome-icon icon="check" />
                                </span>
                            </span>
                        </div>
...
export default {
    ...
    methods: {
        ...
        toggleParentFilter(parent) {
            let newFilter = {
                ...this.filterState
            };

            if (newFilter.parents == null) {
                newFilter.parents = [ parent.id ];
            } else {
                if (newFilter.parents.includes(parent.id)) {
                    newFilter.parents = newFilter.parents.filter(item => item !== parent.id);
                } else {
                    newFilter.parents.push(parent.id);
                }
            }
            this.$emit("updateFilter", newFilter);
        },
        ...
        getExistingGroups() {
            this.$root.getSocket().emit("getGroups", (res) => {
                if (res.ok) {
                    this.parentsList = res.parents;
                }
            });
        },
        ...
        getGroupMonitorCount(parent) {
            return Object.values(this.$root.monitorList).filter(monitor => {
                console.log(monitor);
                return monitor.parents.find(monitorParent => monitorParent.parent_id === parent.id);
            }).length;
        },
        clickMethod() {
